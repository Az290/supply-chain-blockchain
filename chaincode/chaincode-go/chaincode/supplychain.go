...
func (s *SmartContract) CreateProduct(ctx contractapi.TransactionContextInterface, id string, name string, productType string, origin string, ownerID string, batchNumber string, quantity int, unit string, price float64, description string) error {
	...
	if participant != nil && participant.Role != "PRODUCER" && participant.Role != "ADMIN" {
		return fmt.Errorf("only PRODUCER or ADMIN can create products, got role: %s", participant.Role)
	}
...
func (s *SmartContract) UpdateProductStatus(ctx contractapi.TransactionContextInterface, id string, newStatus string, location string, updatedBy string, description string, temperature string, humidity string) error {
	...
	// Validate status
	newOrder, validNew := validStatuses[newStatus]
	currentOrder, validCurrent := validStatuses[product.CurrentStatus]
	if !validNew {
		return fmt.Errorf("invalid status: %s", newStatus)
	}
	if !validCurrent {
		return fmt.Errorf("current status invalid: %s", product.CurrentStatus)
	}
	if newOrder <= currentOrder {
		return fmt.Errorf("cannot change status from %s to %s (must move forward)", product.CurrentStatus, newStatus)
	}

	// Kiem tra quyen cap nhat trang thai
	participant, err := s.GetParticipant(ctx, updatedBy)
	if err == nil && participant != nil {
		allowedStatuses, exists := roleStatusPermission[participant.Role]
		if exists {
			allowed := false
			...
			if !allowed {
				return fmt.Errorf("role %s is not allowed to set status %s", participant.Role, newStatus)
			}
		}
	}
	...
	statusRecord := StatusRecord{
		Status: newStatus, Timestamp: timestamp, Location: location,
...
func (s *SmartContract) DeleteProduct(ctx contractapi.TransactionContextInterface, id string, deletedBy string) error {
	...
	if err == nil && participant != nil && participant.Role != "ADMIN" {
